...
image = 'subdataset_ubiris_v2\C1_S1_I1.tiff'; 
...
%% STEP 3: Chiamata alle funzioni di segmentazione, normalizzazione ed encoding
% Segmentazione Hough
...
subplot(2,1,1); imshow(img_red); hold on;
rectangle('Position', [c_min, r_min, box_width, box_height], 'EdgeColor', 'g', 'LineWidth', 2); 
title('ROI selezionata');

subplot(2,1,2); imshow(img_red); hold on;  title({'Segmentazione', 'Verde = ROI, Rosso = Pupilla, Ciano = Iride'});
...
%% STEP 5: Visualizzazione Normalizzazione ed encoding
figure('Name', 'Normalizzazione');
% Segmentazione 
subplot(3,1,1); imshow(img_red); hold on; title('Segmentazione');
